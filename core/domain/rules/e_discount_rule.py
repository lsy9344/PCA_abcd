"""
E 매장 할인 규칙 - 4_discount_logic.mdc 기반 동적 계산
- 1시간 단위 유료 쿠폰 특화 (B매장과 동일한 구조)
...
class EDiscountRule:
    """E 매장 할인 규칙 - 동적 계산 알고리즘 기반"""
    
    def __init__(self):
        self.logger = logging.getLogger(__name__)
        
        # YAML 설정 파일 로드
        config_path = Path(__file__).parent.parent.parent.parent / "infrastructure/config/store_configs/e_store_config.yaml"
    ...
    def decide_coupon_to_apply(
        ...
        """
        E 매장 쿠폰 적용 개수 결정 - 동적 계산 알고리즘 사용
        E 매장 특징: 1시간 단위 유료 쿠폰, 주말 전용 쿠폰 없음 (PAID fallback)
        ...
        try:
            ...
            self.logger.info(f"[E 매장] {'평일' if is_weekday else '주말'} 쿠폰 계산 시작")
            ...
            self.logger.info(f"[E 매장] 동적 계산 결과: {applications_dict}")
            ...
            for coupon_key, count in applications_dict.items():
                ...
                if config:
                    ...
                    self.logger.info(f"[E 매장] {config.coupon_name}: 계산값 {count}개, 보유 {available}개 → 적용 {actual_count}개")
            
            # 표준 형식으로 변환 (인터페이스 호환)
            result = {'FREE_1HOUR': 0, 'PAID_1HOUR': 0}
            ...
            # E 매장 특징: 1시간 단위 쿠폰으로 인한 추가 로깅
            paid_minutes = result['PAID_1HOUR'] * 60
            free_minutes = result['FREE_1HOUR'] * 60
            
            self.logger.info(f"[E 매장] 최종 적용 계획: {result}")
            self.logger.info(f"[E 매장] 무료쿠폰: {result['FREE_1HOUR']}개 ({free_minutes}분)")
            self.logger.info(f"[E 매장] 유료쿠폰: {result['PAID_1HOUR']}개 ({paid_minutes}분) - 1시간 단위")
            self.logger.info(f"[E 매장] 총 적용 시간: {total_apply_minutes}분 (목표: {target_minutes}분)")
            self.logger.info(f"[E 매장] 목표 달성: {'성공' if total_apply_minutes >= target_minutes else '실패'}")
            
            if not is_weekday:
                self.logger.info("[E 매장] 주말 특징: WEEKEND 쿠폰 없음 → PAID 타입으로 fallback 적용됨")
            
            return result
            
        except Exception as e:
            self.logger.error(f"[실패] E 매장 쿠폰 적용 계산 중 오류: {str(e)}")
            return {'FREE_1HOUR': 0, 'PAID_1HOUR': 0}
    ...
    def _calculate_current_discount(self, my_history: Dict[str, int]) -> int:
        ...
        for coupon_key, count in my_history.items():
            ...
            if config:
                ...
                self.logger.debug(f"[E매장 시간계산] {config.coupon_name}: {count}개 × {config.duration_minutes}분 = {count * config.duration_minutes}분")
            else:
                # 표준 쿠폰 타입 - E매장 특화 계산
                ...
                if coupon_key in ['FREE_30MIN', 'FREE_1HOUR', 'PAID_30MIN', 'PAID_1HOUR', 'PAID_24HOUR']:
                    self.logger.debug(f"[E매장 시간계산] {coupon_key} (표준): {count}개")
        
        self.logger.info(f"[E매장 시간계산] 총 적용된 할인 시간: {total_minutes}분")
