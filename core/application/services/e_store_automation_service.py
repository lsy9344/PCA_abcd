...
class EStoreAutomationService:
    ...
    def __init__(self,
        ...
        # E 매장 설정 로드
        self._store_config = config_manager.get_store_config("E")
        self._playwright_config = config_manager.get_playwright_config()
        
        # E 매장 할인 계산기 생성 (기본 계산기 사용)
    ...
    async def execute(self, request: AutomationRequest) -> AutomationResponse:
        ...
        try:
            ...
            if applications:
                ...
                success_message = f"E 매장 쿠폰 적용 완료: {', '.join(applied_summary)}"
            else:
                success_message = "E 매장: 적용할 쿠폰이 없습니다"
